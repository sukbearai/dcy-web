...
export function usePdf(options: PDFHookOptions): PDFHookReturn {
  const {
  ...
  _pdfjs.GlobalWorkerOptions.workerSrc = `https://cdnjs.cloudflare.com/ajax/libs/pdf.js/${_pdfjs.version}/pdf.worker.js`

  watch(
    file,
    (_file, _oldFile) => {
      if (!_file || _oldFile === _file)
        return
      ...
      try {
        if (element.value) {
          if (renderType === 'canvas')
            renderCanvas(element.value, _PDFPage)
  ...
  const fitWidth = () => {
    if (!pdfPage.value || !element.value || !element.value.parentElement)
      return
  ...
  const nextPage = () => {
    if (page.value === pdfDocument.value?.numPages)
      return
  ...
  const setPage = (_page: number) => {
    if (!pdfDocument.value)
      return
    if (_page > pdfDocument.value.numPages)
      return
    page.value = _page
  }
